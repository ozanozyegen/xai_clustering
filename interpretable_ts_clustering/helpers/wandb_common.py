...
def wandb_save(is_online, disable_code=False, is_silent=False):
    if not is_online:
...
def convert_wandb_config_to_dict(wandb_config: wandb.sdk.wandb_config.Config):
    return dict(wandb_config.items())


def get_wandb_df(project_name, wandb_entity):
...
def restore_wandb_online(project_name, model_id, entity_name='oozyegen',
